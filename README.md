...
- **并发 Key 跑马灯**：时间戳行右侧内联显示「⚡ 并发中：」+ 每个活跃请求的 badge（Key 编号、模型名、已用时间，如 `#3 gpt-5.6-sol 12s`），已用时间每秒实时更新；badge 从右向左累积，溢出时旧 badge 从左侧滚出（`overflow:hidden` + `translateX` 轮显）；无并发时整个 ticker 隐藏，不占空间
